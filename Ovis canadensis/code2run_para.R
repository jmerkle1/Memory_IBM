...
args <- list(c(.05, .10, .20), #increase in probability of death for uninformed
             c(.01, .05, .1), # naive learning probability of the oldest animals (i.e., the ones that have the highest naive learning)
             ...
             c(20, 50, 100), # starting number of individuals
             25, # how many years should the simulation run for?
             0.4, #what is the sex ratio of of the population/births?
             c(4, 8, 12, 16), # lambda value for starting age distribution based on poison distribution
             ...
             "C:/Users/Zach/Desktop/results/Ovis_Can", #an empty folder where results will be saved.
...
time1 <- Sys.time()
results.folders <- paste("C:/Users/Zach/Desktop/results/Ovis_Can", paste0(1:nrow(args), gsub(":", "",strsplit(as.character(Sys.time()), " ")[[1]][2])), sep = "/")
