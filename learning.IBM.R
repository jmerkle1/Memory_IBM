learning <- 
function(nl, curIndividual, ageClass, maxAgeClass = 4, is.alive, boldness, interactionMatrix, ind, densityDependType = 0, indexJ, pastInteractionInds, familiarBias = .1){
  ...
  if(familiarBias > 0){
    if(densityDependType == 1){
      socialPool$numInteractions <- rpois(nrow(socialPool), (si * curIndividual$boldness * length(is.alive)/K * socialPool$boldness) + (pastInteractionInds[-indexJ] * familiarBias))# calculate a social interaction probability for each individual that is alive
    }
    if(densityDependType == 0){
      socialPool$numInteractions <- rpois(nrow(socialPool), (si * curIndividual$boldness * socialPool$boldness) + (pastInteractionInds[-indexJ] * familiarBias))# calculate a social interaction probability for each individual that is alive
    }
    if(densityDependType == -1){
      socialPool$numInteractions <- rpois(nrow(socialPool), (si * curIndividual$boldness * ifelse(length(is.alive)>=K, 1, 1+(1-length(is.alive)/K)) * socialPool$boldness) + (pastInteractionInds[-indexJ] * familiarBias))# calculate a social interaction probability for each individual that is alive
    }
  }
  else{
    if(densityDependType == 1){
