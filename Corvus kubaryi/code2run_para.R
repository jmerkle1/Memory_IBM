...
birthdeath.file <- "C:/Users/Zach/Documents/GitHub/Memory_IBM/Memory_IBM/Sitophilus oryzae/ageClass_SitoOryz.csv" #dataframe of age based birth and death rate for FEMALES only. The columns should be age, ageClass, birthRate, and survivalRate, in that order.
...
args <- list(c(.05, .10, .20), #increase in probability of death for uninformed
             ...
             c(0, 2, 5), # maximum mean (i.e., lambda of poison distribution) number of interactions per pair (if animal has 1 bold, it interacts with an animal with 1 boldness, and population is at or above K, this is the lambda of the interaction distributions)
             c(0.01, .03, .05, .10), # given an interaction, what is the probability that information is transfered (min=0, max=1)
             40000, # carrying capacity
             c(1000, 4000, 8000), # starting number of individuals
             25, # how many years should the simulation run for?
             0.5, #what is the sex ratio of of the population/births?
             c(2, 4, 6), # lambda value for starting age distribution based on poison distribution
             ...
             "C:/Users/Zach/Desktop/results/SitoOry", #an empty folder where results will be saved.
...
time1 <- Sys.time()
results.folders <- paste("C:/Users/Zach/Desktop/results/SitoOry", paste0(1:nrow(args), gsub(":", "",strsplit(as.character(Sys.time()), " ")[[1]][2])), sep = "/")
...
sfClusterApplyLB(1:nrow(args), function(i){
  ...
  write.csv(paste0(args[i,12],"/params.csv"), arg[i,])
  })
